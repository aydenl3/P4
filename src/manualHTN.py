...
# function to collect wood using a wooden axe  
# requirements:  
# - player must have at least 2 time  
# - player must have at least 1 wooden axe  
# what happens:  
# - player gets 1 wood  
# - player loses 2 time  
def op_wooden_axe_for_wood(state, ID):  
    if state.time[ID] >= 2 and state.wooden_axe[ID] >= 1:  
        ...
        state.time[ID] -= 2  
        return state  
    return False  # return false if player doesn't have enough time or axe  

# function to craft planks from wood  
# requirements:  
# - player must have at least 1 time  
# - player must have at least 1 wood  
# what happens:  
# - player gets 4 planks  
# - player loses 1 wood  
# - player loses 1 time  
def op_craft_plank(state, ID):  
    if state.time[ID] >= 1 and state.wood[ID] >= 1:  
        ...
        state.time[ID] -= 1  
        return state  
    return False  # return false if player doesn't have enough time or wood  

# function to craft sticks from planks  
# requirements:  
# - player must have at least 1 time  
# - player must have at least 2 planks  
# what happens:  
# - player gets 4 sticks  
# - player loses 2 planks  
# - player loses 1 time  
def op_craft_stick(state, ID):  
    if state.time[ID] >= 1 and state.plank[ID] >= 2:  
        ...
        state.time[ID] -= 1  
        return state  
    return False  # return false if player doesn't have enough time or planks  

# function to craft a crafting bench  
# requirements:  
# - player must have at least 1 time  
# - player must have at least 4 planks  
# what happens:  
# - player gets 1 crafting bench  
# - player loses 4 planks  
# - player loses 1 time  
...
pyhop.declare_operators (op_punch_for_wood, op_wooden_axe_for_wood, op_craft_plank, op_craft_stick, op_craft_bench, op_craft_wooden_axe_at_bench)
...
def produce_wood_with_axe(state, ID):
	return [('have_enough', ID, 'wooden_axe', 1), ('op_wooden_axe_for_wood', ID)]


pyhop.declare_methods ('produce_wood', produce_wood_with_axe, punch_for_wood)
